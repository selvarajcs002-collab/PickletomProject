  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (onboardingData.localProfileImage) {
        ...
        const res = await profileService.uploadSingle(onboardingData.localProfileImage, 'profile');
        if (res && res.status === 1) {
          profileUrl = res.url;
      ...
      if (onboardingData.localCoverImage) {
        ...
        const res = await profileService.uploadSingle(onboardingData.localCoverImage, 'cover');
        if (res && res.status === 1) {
          coverUrl = res.url;
      ...
      if (onboardingData.localMediaFiles.length > 0) {
        ...
        const res = await profileService.uploadMultiple(onboardingData.localMediaFiles);
        if (res && res.status === 1) {
